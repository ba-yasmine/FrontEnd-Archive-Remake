...
const DemandePreview = ({lotID}) => {
  ...
  const {userAuth,setUserAuth} = useContext(UserContext);
    ...
    useEffect(() => {
      setheaderTitle('Informations du lot ' + params.lotID)
      setheaderArrow(true)
    })
 ...
 useEffect(()=>{
            ...
            axios.get(`${process.env.REACT_APP_HOST_VAR}rest/batches/getBatches/${userAuth.company}/${params.lotID}`,{ headers:{
    ...
    return (
      <div >
        ...
        <h4 style={{marginBottom:"30px", color:"#0243cd", textAlign:"center"}}> les boites associées</h4>
      ...
      <td>{el.codeBarre !== null ? el.codeBarre :  "non attribué"}</td>
    ...
    <Button style={{border:"none", background:"none", color:"#0243cd"}} variant="success" onMouseEnter={()=>setFullDescription(el.description)}>{fulldescription}</Button>
          ...
          <td>{el.departements?.nom !== undefined ? el?.departements.nom :  "non attribuée"}</td>
