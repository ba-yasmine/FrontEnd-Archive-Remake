...
import {PanierRequest} from "./BoxRequest"
import Card from 'react-bootstrap/Card';
import {TiDeleteOutline} from 'react-icons/ti'
function PanierBoxRequest({provenanceId, datePicker, company}) {

 
  const {selectionRequest,setSelectionRequest} = useContext(PanierRequest);
    const [lgShow, setLgShow] = useState(false);
    const [count, setPanier] = useState(0);
    const handleClose = () => {setLgShow(false);document.location.reload();setSelectionRequest([])}
...
//envoyer les boites selectionnées
    const sendRequ=() => {
          ...
          setLoading(true)
         axios.post(`${process.env.REACT_APP_HOST_VAR}rest/batches/createBatch/Box_Request/${company}/${userAuth.ID}/${provenanceId}/${datePicker.toISOString()}`,selectionRequest,{ headers:{
