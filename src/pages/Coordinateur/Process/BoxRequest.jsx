...
import PanierBoxRequest from "./panierBoxRequest"
...
export const PanierRequest = createContext();
const BoxRequest = () => {
...
useEffect(() => {
  setheaderTitle('Demande Boxes-Request')
  setheaderArrow(true)
})
...
const handleDateState = (date) => {
  ...
  useEffect(() => {
    const getProvenancesPromise = axios.get(`${process.env.REACT_APP_HOST_VAR}rest/provenance/getProvenances/${company}`, {
      headers: {
        'Authorization':  "Bearer" + userAuth.token
      }
    });
    
    const getDepartementsPromise = axios.get(`${process.env.REACT_APP_HOST_VAR}rest/departements/getDepartements/${company}`,{headers:{
      'Authorization':  "Bearer" + userAuth.token
    }})
  ...
  }, [searchTerm, departement, currentPage, sortString, location, itemsPerPage, selectedProventreetrance])
  ...
  return (
    ...
    <div style={{backgroundColor:'white', padding:'10px'}}>
       <div style={{width:'100%' ,padding:`${isMobile ? '10px 10px' : '10px 30px'}`}}>
    ...
    {provenance?.map((el,id)=>{
     ...
     return <option key={id} value={el.id}>Provenance: {el.adresse}</option>
        ...
        <Form.Select  aria-label="Default select example" onChange={handledepartementChange}>
      <option value=''>Departement Tous</option>
      {departementList?.map(el => (
        <option value={el.id}>Departement {el.nom}</option>
      ))}
    ...
    <div >
<PanierRequest.Provider value={{selectionRequest,setSelectionRequest}}>
<PanierBoxRequest provenanceId={selectedProventreetrance} datePicker={datePickDelivery} company={company} />
</PanierRequest.Provider>
   </div>
        </div>
